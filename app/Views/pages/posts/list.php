...
<div class="card rounded-0">
    <div class="card-header">
        <div class="d-flex w-100 justify-content-between">
            ...
            <div class="col-auto">
                <a href="<?= base_url('/post_add') ?>" class="btn btn btn-primary bg-gradient rounded-0"><i
                        class="fa fa-plus-square"></i> Add Post</a>
            </div>
        </div>
    </div>
    <div class="card-body">
                ...
                <div class="col-lg-6 col-md-8 col-sm-12 col-xs-12">
                    <form action="<?= base_url("Main/posts") ?>" method="GET">
                        <div class="input-group">
                            <input type="search" id="search" name="search" placeholder="Search blog here."
                                value="<?= $request->getVar('search') ?>" class="form-control">
            ...
            <table class="table table-striped table-bordered">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="px-2 py-1 align-middle"><?= date("M d, Y h:i A", strtotime($row['updated_at'])) ?>
                        </td>
                        <td class="px-2 py-1 align-middle"><?= ($row['title']) ?><a
                                href="<?= base_url("Blog/view/".$row['id']) ?>" target="_blank"
                                class="mx-1 text-muted text-decoration-none"><i class="fa fa-external-link"></i></a>
                        </td>
                        ...
                        <td class="px-2 py-1 align-middle text-center">
                            <a href="<?= base_url('/post_edit'.$row['id']) ?>"
                                class="mx-2 text-decoration-none text-primary"><i class="fa fa-edit"></i></a>
                            <a href="<?= base_url('/post_delete'.$row['id']) ?>"
                                class="mx-2 text-decoration-none text-danger"
                                onclick="if(confirm('Are you sure to delete  <?= $row['title'] ?> from list?') !== true) event.preventDefault()"><i
