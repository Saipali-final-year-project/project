<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <title>Invoice Check |</title>

    <style>
    .invoice-box {
        max-width: 800px;
        margin: auto;
        padding: 30px;
        border: 1px solid #eee;
        box-shadow: 0 0 10px rgba(0, 0, 0, 0.15);
        font-size: 16px;
        line-height: 24px;
        font-family: "Helvetica Neue", "Helvetica", Helvetica, Arial, sans-serif;
        color: #555;
        margin-top: 50px;
    }

    .invoice-box table {
        width: 100%;
        line-height: inherit;
        text-align: left;
    }

    .invoice-box table td {
        padding: 5px;
        vertical-align: top;
    }

    .invoice-box table tr td:nth-child(n + 2) {
        text-align: right;
    }

    .invoice-box table tr.top table td {
        padding-bottom: 20px;
    }

    .invoice-box table tr.top table td.title {
        font-size: 45px;
        line-height: 45px;
        color: #1ba70c;
    }
    .invoice-box table tr.top table td.title a{
        color: #1ba70c;
    }

    .invoice-box table tr.information table td {
        padding-bottom: 40px;
    }

    .invoice-box table tr.heading td {
        background: #eee;
        border-bottom: 1px solid #ddd;
        font-weight: bold;
    }

    .invoice-box table tr.details td {
        padding-bottom: 5px;
    }

    .invoice-box table tr.item td {
        border-bottom: 1px solid #eee;
    }

    .invoice-box table tr.item.last td {
        border-bottom: none;
    }

    .invoice-box table tr.item input {
        padding-left: 5px;
    }

    .invoice-box table tr.item td:first-child input {
        margin-left: -5px;
        width: 100%;
    }

    .invoice-box table tr.total td:nth-child(2) {
        border-top: 2px solid #eee;
        font-weight: bold;
    }

    .invoice-box input[type="number"] {
        width: 60px;
    }

    @media only screen and (max-width: 600px) {
        .invoice-box table tr.top table td {
            width: 100%;
            display: block;
            text-align: center;
        }

        .invoice-box table tr.information table td {
            width: 100%;
            display: block;
            text-align: center;
        }
    }

    /** RTL **/
    .rtl {
        direction: rtl;
        font-family: Tahoma, "Helvetica Neue", "Helvetica", Helvetica, Arial,
            sans-serif;
    }

    .rtl table {
        text-align: right;
    }

    .rtl table tr td:nth-child(2) {
        text-align: left;
    }
    </style>

</head>

<body> 
    <script src="https://checkout.flutterwave.com/v3.js"></script>
    <!-- partial:index.partial.html -->
    <div id="payment-failed">
    </div>
    <div class="invoice-box">
        <form>
            <table cellpadding="0" cellspacing="0">
                <tr class="top">
                    <td colspan="4">
                        <table>
                            <tr>
                                <td class="title">
                                    <a href="<?php echo base_url('/'); ?>">TOOKE</a>
                                </td>

                                <td>
                                    Invoice No: 123<br> Created:
                                    <?php $datetime = new DateTime("now", new DateTimeZone("Africa/Kampala")); echo $datetime->format("d-M-y h:ia");?>
                                </td>
                            </tr>
                        </table>
                    </td>
                </tr>

                <tr class="information">
                    <td colspan="4">
                        <table>
                            <tr>
                                <td>
                                    Sparksuite, Inc.<br> 12345 Sunny Road<br> Sunnyville, CA 12345
                                </td>

                                <td>
                                    Payment Status:
                                    <span>Pending</span>
                                </td>
                            </tr>
                        </table>
                    </td>
                </tr>

                <tr class="heading">
                    <td colspan="3">Customer Info</td>
                    <td colspan="3"></td>
                </tr>

                <tr class="details">
                    <td colspan="3">Name: <?=  $firstname?> <?=  $lastname?> </td>
                    <td colspan="3">Mobile no: <?=  $phone?> </td>
                </tr>
                <tr class="details">

                    <td colspan="3">Postcode/NIN: <?=  $postcode?></td>
                    <td colspan="3">Address: <?=  $address?></td>
                </tr>
                <tr class="details">

                    <td colspan="3">Email: <?=  $email?></td>

                </tr>

                <tr class="heading">
                    <td>Item</td>
                    <td>Unit Cost</td>
                    <td>Quantity</td>
                    <td>Price</td>
                </tr>

                <?php if ($items):?>
                <?php foreach($items as $item):?>
                <tr class="item" v-for="item in items">
                    <td><?=  $item['name']?></td>
                    <td>UGX <?= $item['price'] ?></td>
                    <td><?=  $item['quantity']?></td>
                    <td> UGX <?= $item['price'] * $item['quantity']?></td>
                </tr>
                <?php endforeach; ?>
                <?php endif; ?>

                <tr class="total">
                    <td colspan="3"></td>
                    <td>Total: UGX <?= $Total ?></td>
                </tr>
            </table>
            <button type="button" onclick="makePayment()">Pay Now</button>
        </form>
    </div>

    <script src="https://unpkg.com/sweetalert/dist/sweetalert.min.js"></script>
    <script>
    function makePayment() {
        FlutterwaveCheckout({
            public_key: "FLWPUBK_TEST-6034f68122f05f5acb7527c3e3c7d829-X",
            tx_ref: "titanic-48981487343MDI0NzMx",
            amount: <?= $Total ?>,
            currency: "UGX",
            country: "UG",
            payment_options: " mobilemoneyuganda, card, ussd",
            callback: function(payment) {
                // Send AJAX verification request to backend
                verifyTransactionOnBackend(payment.id);
            },
            onclose: function(incomplete) {
                if (incomplete || window.verified === false) {
                    swal({
                        title: "SORRY!!",
                        text: "Transaction Failed!",
                        icon: "error",
                        button: "OK",
                    });
                } else {
                    document.querySelector("form").style.display = 'none';
                    if (window.verified == true) {
                        redirect_url: // specified redirect URL
                            "https://callbacks.piedpiper.com/flutterwave.aspx?ismobile=34";
                    }
                    else {
                        document.querySelector("#payment-pending").style.display = 'block';
                    }
                }
            },
            meta: {
                consumer_id: 23,
                consumer_mac: "92a3-912ba-1192a",
            },
            customer: {
                email: "<?= $email ?>",
                phone_number: "<?= $phone ?>",
                name: "<?= $firstname ?> <?= $lastname ?>",
            },
            customizations: {
                title: "OGANIC",
                description: "Payment for an awesome cruise",
                logo: "<?= base_url();?>/assets/img/logo.png",
            },
        });
    }

    function verifyTransactionOnBackend(transactionId) {
        // Let's just pretend the request was successful
        setTimeout(function() {
            window.verified = true;
        }, 200);
    }
    </script>
</body>

</html>